...
- ~~voicedesign 长文本截断（已修复）~~：模型对长输入会**随机提前终止**（同一输入两次合成时长可差一倍）。**Worker 已在端侧根治**：>300 字输入自动按句子边界分块（≤300 字/块）并行合成、mp3 盲拼返回，实测 1083 字 4 块完整输出。Reeden「单次合成最大字符数」可设 **1000~3000**（不再依赖 200 字小分段）。响应头 `X-Chunks` 显示实际分块数。
